...
void World::updateMapCollision()
{
	...
	Command enemy_map_collision;
	enemy_map_collision.category = Category::Enemy;
	enemy_map_collision.action = get_function<Enemy>([this](Enemy &enemy, sf::Time dt)
	{
		collider.checkMapCollision(enemy);
	});
	command_queue.push(enemy_map_collision);
}
...
void  World::luaObjectsToNil()
{
	...
	for (auto *i : World::current_level->getSceneVector())
	{
		if (i->isRemovable() == true || is_new_level)
		{
			...
			if (i->getCategory() == Category::Enemy)
			{
				lua_getglobal(script_system.getLuaState(), "setToNil");
				tolua_pushusertype(script_system.getLuaState(), (void*)i, "Enemy");
				lua_call(script_system.getLuaState(), 1, 1); //1 argumento 1 resultado
			}
			if (i->getCategory() == Category::Door)
			{
				//lua_getglobal(script_system.getLuaState(), "setToNil");
				//tolua_pushusertype(script_system.getLuaState(), (void*)i, "Door");
				//lua_call(script_system.getLuaState(), 1, 1); //1 argumento 1 resultado
			}
		}	
	}
...
int World::tiledGetDoor(lua_State *L)
{
	std::string key = tolua_tostring(L, 1, NULL);
	Door* door = World::current_level->getSceneGraph()->getNode<Door>(key);
	tolua_pushusertype(script_system.getLuaState(), door, "Door");
	return 1;
}
